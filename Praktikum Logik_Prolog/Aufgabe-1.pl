...
% tante/onkel
tanteOnkel(X, TanteOnkel) :- elternteile(X, Elternteil), alleGeschwister(Elternteil, TanteOnkel).
tante(X, Tante) :- tanteOnkel(X, Tante), weiblich(Tante).
onkel(X, Onkel) :- tanteOnkel(X, Onkel), maennlich(Onkel).

% Cousin/Cousine
cousinCousine(X, CousinCousine) :- tanteOnkel(X, TanteOnkel), kinder(TanteOnkel, CousinCousine).
cousin(X, Cousin) :- cousinCousine(X, Cousin), maennlich(Cousin).
cousine(X, Cousine) :- cousinCousine(X, Cousine), weiblich(Cousine).

% Groseltern
groszeltern(X, Groszeltern) :- elternteile(X, Eltern), elternteile(Eltern, Groszeltern).
